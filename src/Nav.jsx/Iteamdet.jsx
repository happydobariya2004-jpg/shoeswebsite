// import React from 'react'
// import Header from '../Header'
// import { useLocation, } from 'react-router-dom'
// import { allData } from './Alld'


// const Iteamdet = () => {


//   let location=useLocation()
//   let currentID=location.pathname.split('/')[8]
//   // let showData=allData.filter((v)=>v.id==currentID)[0]
//   let showData = allData.find((v) => v.i == currentID)[0]

//   return (
//     <>
//       <Header />

//       <h1 className='text-center mt-3'>Iteam Details</h1>

//       <div className="container text-center">
//         <img className='w-25' src={showData.image} alt="" />
//         <h2>{showData.price}</h2>
//         <h2>{showData.details}</h2>


//         <h3>{}</h3>
//       </div>
//     </>
//   )
// }

// export default Iteamdet

import React from 'react'
import Header from '../Header'
import { useParams } from 'react-router-dom'
import { allData } from './Alld'



const Iteamdet = () => {

  const { id } = useParams()

  let showData = allData.find((v) => v.id == id)

  if (!showData) {
    return <h2 className="text-center mt-5">Item not found</h2>
  }

  return (
    <>
      <Header />

      {/* <h4 className='text-center mt-3  '></h4>

      <div className="container ">
        <img className='w-25' src={showData.image} alt="" />

        <h2>{showData.details}</h2>
        <h2 className='star' style={{ color: "green" }}>{showData.rating}</h2>

        <button className='text mb-1' style={{ background: "green", color: "white" }}>Hot Deal</button>
        <h6><del className='color text-danger'>$4000</del>   ${showData.price}</h6>



        <h6>select size <span className='text' style={{ color: "blue" }}>size chat</span></h6>
        <div className="col-lg-7 justify-content-end d-flex mt-3">
          <div className="d-flex gap-2 ">

            <span className="border border-dark px-2 py-1">7</span>
            <span className="border border-dark px-2 py-1">8</span>
            <span className="border border-dark  px-2 py-1">9</span>
            <span className="border border-dark px-2 py-1">10</span>
          </div>

        </div>


        <div className="row mt-3">

          <div className="col-lg-6  justify-content-end d-flex">

            <button className='btn' style={{background:"green",color:"white"}}>Add Cart</button>

          </div>
          <div className="col-lg-6 justify-content-start d-flex" >
            <button className='btn'style={{background:"green", color:"white"}} >Buy Now</button>


          </div>
        </div>
        
      </div>
      <Foter/> */}

      <div className="container-fluid">
         <div className="container ">
      <div className="row">
        <div className="col-12 col-md-6 d-flex">
          <img className=" w-50 rounded-5 m-5" src={showData.image} alt="" />

          <div className="col-lg-6  m-5" style={{ marginLeft: "100px" }}>
            <h2>{showData.details}</h2>
            <h2>${showData.price}</h2>
            <h2 className="rating " style={{ color: "#FF6B2B" }}>{showData.rating}</h2>
            <button className='text mb-1 border rounded-1' style={{ background: "#ff6b2b", color: "white" }}>Hot Deal</button>
            <h6 className='fs-3'><del className='color text-danger fs-5'>₹4000</del> ₹{showData.price}</h6>



            <h6>select size <span className='text' style={{ color: "#ff6b2b" }}>size chat</span></h6>
            {/* <div className="col-lg-7 justify-content-end d-flex mt-3"> */}
            <div className="d-flex  gap-3 text-text-border-success-subtle">

              <span className="border border-dark px-2 py-1 rounded-2 ">7</span>
              <span className="border border-dark px-2 py-1 rounded-2 ">8</span>
              <span className="border border-dark  px-2 py-1 rounded-2">9</span>
              <span className="border border-dark px-2 py-1 rounded-2">10</span>
            </div>
            <div className="col-6 d-flex gap-3 mt-4  w-100">
  <button className='bt'>Add Cart</button>

       <button className='bt'>Buy Now</button>

       
            </div>



          </div>
          
        </div>
       
        <div className="row" style={{ marginTop: "100px" }}>
        
          <div className="col-sm-6 col-md-4 col-lg-3">
            <img src="data:image/jpeg;base64," alt="" />
            <h6>Men strappy Floater sandals</h6>
            <p><del className="text-danger"> $3200</del> $1900</p>
          </div>
          <div className=" col-lg-3 col-sm-4 col-md-3 1875rem ">
            <img src="https://encrypted-tbn1.gstatic.com/shopping?q=tbn:ANd9GcTqHOxSftVRu5LrEMoIyJRuLHUChmsuRUYCQc6QJgmb1XO7lXwjdH7X3EazJDVmNgd1JonMGf2gMO8pXY7o5mFI1qmfMBZZhagnZwabIbFg7GKu0LIlEZpQ-2nNBviLwOx47OOXlA&usqp=CAc" alt="" />
            <h6>unning / Sports shoe Grey with white sole</h6>
             <p><del className="text-danger"> $5200</del> $2900</p>
          </div>
          <div className="col-lg-3 col-sm-4 col-md-3">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQMlK58_-Pd85yYTs2PdRxgXoR4dacSqFzfPQZwumhYCEIl45IiJRU5TxQ&s" alt="" />
            <h6>Slip-on / Walking shoes   Mix of green, yellow, white, and red</h6>
             <p><del className="text-danger"> $2200</del> $1100</p>
          </div>
          <div className="col-lg-3 col-sm-4 col-md-3">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSA_gEyyG4sGOZE76zCFvgmEUe_QTLd7dokTTxPpkuIrS43B7GPe7ShVj29YJw_KZYr4f9VqQ&s" alt="" />
            <h6>lip-on / Walking shoe Light grey with white sole</h6>
             <p><del className="text-danger"> $4200</del> $3200</p>
          </div>
        </div>
      </div>
    </div>
      </div>
    </>
  )
}

export default Iteamdet